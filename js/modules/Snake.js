/**
 * Gerencia toda a lógica relacionada à cobra
 * Movimentação, crescimento, colisões e renderização da cobra
 */
...
export class Snake {
    ...
    update() {
        ...
        switch (this.direction) {
            case DIRECTIONS.UP: head.y -= GAME_CONFIG.GRID_SIZE; break;
            case DIRECTIONS.DOWN: head.y += GAME_CONFIG.GRID_SIZE; break;
            case DIRECTIONS.LEFT: head.x -= GAME_CONFIG.GRID_SIZE; break;
            case DIRECTIONS.RIGHT: head.x += GAME_CONFIG.GRID_SIZE; break;
        }
    ...
    draw(ctx) {
        this.segments.forEach((segment, index) => {
            ...
            const segmentColorIndex = (this.currentColorIndex + index) % snakeColors.length;
            const color = index === 0 ?
            ...
            ctx.beginPath();
            roundRect(ctx, segment.x, segment.y, GAME_CONFIG.GRID_SIZE, GAME_CONFIG.GRID_SIZE, 5);
            ctx.fill();
            ctx.stroke();
