/**
 * Gerencia a animação de introdução do jogo
 * Cobras que se movem aleatoriamente e reagem ao mouse
 */
...
export class IntroAnimation {
    ...
    start() {
        // Criar cobras
        this.introSnakes = Array.from({ length: 15 }, () => new IntroSnake(this.canvas, this.mouse));
...
class IntroSnake {
    ...
    update() {
        ...
        // Rebater nas bordas
        if (this.x < 0 || this.x > this.canvas.width) this.direction = Math.PI - this.direction;
        if (this.y < 0 || this.y > this.canvas.height) this.direction = -this.direction;
